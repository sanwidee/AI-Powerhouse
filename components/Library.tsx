                    ...
                    {selectedPost.history.length === 0 ? (
                      ...
                      selectedPost.history.map((log) => (
                        <div key={log.id} className="p-4 rounded-3xl bg-slate-900/50 border border-slate-800 space-y-4 group hover:border-slate-600 transition-all">
                          ...
                          <div className="aspect-square rounded-2xl overflow-hidden border border-slate-800/50 bg-black cursor-pointer" onClick={() => onUpdatePost({...selectedPost, imageSource: log.image})}><img src={log.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" /></div>
